...
class ThrustControl:
    ...
    def apply_thrust(self, lander, duration):
        ...
        lander.fuel_left -= fuel_used
        lander.mass = lander.empty_mass + lander.fuel_left
...
class DoomLander:
    ...
    def init_from_db(self, lander_data):
        """ Initialize lander with database values """
        self.state = LanderState(empty_mass=1200.0, initial_fuel=lander_data["fuel_left"])
        self.state.altitude = lander_data["altitude"]
        self.state.velocity.y = lander_data["velocity"]
        self.thruster = ThrustControl()
...
def run_lunar_landing_simulation():
    ...
    while not doom_lander.has_landed() and iteration_count < MAX_ITERATIONS:
        ...
        remaining_fuel = doom_lander.get_fuel_left()
